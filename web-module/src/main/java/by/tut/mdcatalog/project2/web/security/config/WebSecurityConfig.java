...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests()
                .antMatchers( "/users/**", "/reviews", "/reviews/delete")
                .hasAuthority(ADMIN_ROLE_NAME)
                .antMatchers("/articles/{\\d+}", "/articles", "/orders/**", "/items/**")
                .hasAnyAuthority(CUSTOMER_ROLE_NAME, SALE_ROLE_NAME)
                .antMatchers("/profile/**", "/reviews/add")
                .hasAuthority(CUSTOMER_ROLE_NAME)
                .antMatchers("/articles/add", "/articles/comments")
                .hasAuthority(SALE_ROLE_NAME)
                .antMatchers("/", "/login", "/about", "/images/**", "/messages/**")
                .permitAll()
                .and()
                .formLogin()
                .loginPage("/login")
